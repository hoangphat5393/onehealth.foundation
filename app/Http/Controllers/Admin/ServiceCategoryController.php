...
class ServiceCategoryController extends Controller
{
    ...
    {
        $categories = Category::where(['type' => 'service', 'parent' => 0])->orderBy('sort', 'asc')->paginate(20);

        $total_item = $categories->count();

        return view('admin.service-category.index', compact('categories', 'total_item'));
    }


    public function create()
    {
        return view('admin.service-category.single', ['data' => $this->data]);
    }
    ...
    public function post(Request $request)
    {
        ...
        if ($save == 'apply') {
            ...
            $url = route('admin.serviceCategoryEdit', array($post_id));
            Helpers::msg_move_page($msg, $url);
        } else {
            return redirect(route('admin.serviceCategoryList'));
        }
    }
}
